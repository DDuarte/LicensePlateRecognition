...
void DBViewer::newCarSave()
{
    if (registrationLine->text().remove("-").isEmpty()) {
        QMessageBox::warning(newCarWindow, tr("Erro"), tr("Tem de introduzir uma matrícula"), QMessageBox::Ok);
        return; }
    if (ownerLine->text().isEmpty()) {
        QMessageBox::warning(newCarWindow, tr("Erro"), tr("Tem de introduzir o nome do proprietário"), QMessageBox::Ok);
        return; }
    ...
    if (yearSpinBox->text().isEmpty()) {
        QMessageBox::warning(newCarWindow, tr("Erro"), tr("Tem de introduzir o ano de compra do automóvel"), QMessageBox::Ok);
        return; }
    if (brandComboBox->currentText().isEmpty()) {
        QMessageBox::warning(newCarWindow, tr("Erro"), tr("Tem de introduzir a marca do automóvel"), QMessageBox::Ok);
        return; }
    if (modelLine->text().isEmpty()) {
        QMessageBox::warning(newCarWindow, tr("Erro"), tr("Tem de introduzir o modelo do automóvel"), QMessageBox::Ok);
        return; }
	...
	{
		QMessageBox::information(newCarWindow, tr("Sucesso"), tr("Registo de automóvel adicionado."), QMessageBox::Ok);
		newCarReset();
	}
	else
